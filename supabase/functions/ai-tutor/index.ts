...
serve(async (req) => {
  ...
  try {
    const { exerciseId, studentLevel, mode, countryCode, grade } = await req.json();
    ...
    // RAG: fetch semantically similar content
    let ragContext = "";
    try {
          ...
          if (similar?.length) {
            ragContext =
              ...
              similar
                .map(
                  (s: any) =>
                    `[${s.content_type}] (تشابه ${(s.similarity * 100).toFixed(0)}%): ${s.content_text?.slice(0, 200)}`,
                )
                .join("\n\n");
          }
        }
    ...
    const levelDesc =
      studentLevel === "beginner"
        ? "مبتدئ — اشرح ببساطة شديدة مع أمثلة"
        : studentLevel === "intermediate"
          ? "متوسط — اشرح بوضوح مع بعض التفصيل"
    ...
    const countryName = COUNTRY_NAMES[countryCode || "DZ"] || countryCode || "الجزائر";

    const prompt = `أنت مدرّس رياضيات لمنهاج ${countryName}. تتحدث بالعربية الفصحى البسيطة.

## التمرين:
${exercise.text}

## البلد: ${countryName} · المستوى: ${grade || exercise.grade || "غير محدد"}
...
- اجعل الشرح ملائماً لتلميذ من ${countryName} ولمصطلحات منهجه الرسمي`;

    const response = await callGemini([{ role: "user", parts: [{ text: prompt }] }], {
      systemInstruction: `أنت مدرّس رياضيات خبير متخصص في منهاج ${countryName}. تشرح بالعربية بوضوح وبساطة وفق مفردات المنهج الرسمي.`,
      temperature: 0.2,
    });
