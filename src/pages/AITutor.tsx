...
export default function AITutor() {
  ...
  const types = useMemo(
    () => [...new Set(exercises.map((e) => e.type).filter((t) => t && t !== "unclassified"))].sort(),
    [exercises],
  );
  ...
  const askTutor = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("ai-tutor", {
        body: { exerciseId: selectedEx.id, studentLevel, mode, countryCode, grade: gradeFilter },
      });
  ...
  const renderExplanation = (text: string) => {
    ...
    return lines.map((line, i) => {
      ...
      if (line.startsWith("**") && line.endsWith("**"))
        return (
          <p key={i} className="text-sm font-bold text-foreground my-1">
            {line.replace(/\*\*/g, "")}
          </p>
        );
  ...
  return (
    <div className="min-h-screen bg-background" dir="rtl">
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(to left, hsl(var(--accent) / 0.08), hsl(var(--accent) / 0.03), hsl(var(--background)))",
        }}
      >
        <div className="max-w-6xl mx-auto">
          ...
          <p className="text-muted-foreground text-sm">
            اختر تمريناً → يشرحه لك خطوة بخطوة بالعربية مع ذكر المفهوم المستخدم في كل خطوة
          </p>
        </div>
      </div>
            ...
            <div className="overflow-y-auto space-y-1.5" style={{ maxHeight: "calc(100vh - 380px)" }}>
                ...
                >
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-muted text-muted-foreground">
                      {GRADE_LABELS[ex.grade] || ex.grade}
                    </span>
              ...
              {filtered.length === 0 && !loading && (
                <div className="text-center py-8 text-xs text-muted-foreground">
                  لا توجد تمارين للمستوى المحدد
                  {gradeFilter && <div className="mt-1">({GRADE_LABELS[gradeFilter]})</div>}
                </div>
              )}
            </div>
          </div>
          ...
          <div className="flex-1 space-y-4">
            {selectedEx ? (
              <>
                <div className="p-5 rounded-xl border border-border bg-card">
                  <div className="flex items-center gap-2 mb-3">
                    <span className="text-[10px] px-2 py-0.5 rounded-full bg-primary/10 text-primary font-bold">
                      {GRADE_LABELS[selectedEx.grade] || selectedEx.grade}
                    </span>
                    <span className="text-[10px] px-2 py-0.5 rounded-full bg-muted text-muted-foreground">
                      {TYPE_LABELS[selectedEx.type] || selectedEx.type}
                    </span>
                    {countryCode && (
                      <span className="text-[10px] px-2 py-0.5 rounded-full bg-muted text-muted-foreground">
                        {countryCode}
                ...
                {kbLoading ? (
                  ...
                  <div className="p-5 rounded-xl border border-primary/20 bg-primary/5">
                    ...
                    {kbPattern?.description && (
                      <p className="text-xs text-muted-foreground mb-4 border-r-2 border-primary/30 pr-3">
                        {kbPattern.description}
                      </p>
                    )}
                    ...
                    {kbDecon.needs && kbDecon.needs.length > 0 && (
                      <div className="mt-4 pt-3 border-t border-primary/10">
                        <h4 className="text-[10px] font-bold text-muted-foreground mb-2 uppercase tracking-wider">
                          المتطلبات المسبقة
                        </h4>
                        <div className="flex gap-1.5 flex-wrap">
                          {kbDecon.needs.map((need: string, i: number) => (
                            <span
                              key={i}
                              className="text-[10px] px-2 py-1 rounded-full bg-accent/10 text-accent-foreground font-medium"
                            >
                              {need}
                            </span>
                          ))}
                ...
                {explanation && (
                  <div
                    className="p-6 rounded-xl border border-primary/20 bg-primary/5 overflow-y-auto"
              ...
              <div className="flex items-center justify-center h-full">
                <div className="text-center space-y-4 max-w-sm">
                  ...
                  <p className="text-sm text-muted-foreground">
                    اختر تمريناً من القائمة وسيقوم المدرّس الآلي بشرحه لك خطوة بخطوة مع ذكر المفهوم المستخدم في كل خطوة.
                  </p>
                </div>
              </div>
            )}
      ...
      <div className="fixed bottom-4 left-4 flex gap-2 z-50">
        <a
          href="/"
          className="px-4 py-2 rounded-lg text-xs font-bold bg-card border border-border text-foreground hover:bg-accent transition-all"
        >
          🏠 الرئيسية
        </a>
        <a
