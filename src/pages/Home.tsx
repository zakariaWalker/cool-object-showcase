      ...
      <section className="relative pt-20 pb-12 px-6">
        ...
        <div className="relative max-w-5xl mx-auto space-y-4">
          ...
          <p className="text-muted-foreground text-sm">
            أكملت <strong className="text-foreground">{completedCount} من 4</strong> مراحل — واصل من حيث توقفت.
          </p>
        </div>
      </section>

      {/* Main Content */}
      <section className="max-w-6xl mx-auto px-6 pb-24 grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-8 order-1 lg:order-2 space-y-8">
          <div className="flex items-center justify-between">
            <h2 className="text-2xl font-black">رحلتك</h2>
            <div className="flex -space-x-2 space-x-reverse">
              {[1, 2, 3, 4].map((n) => (
                <div
                  key={n}
                  className={`w-8 h-8 rounded-full border-2 border-background flex items-center justify-center text-[10px] font-black shadow-sm ${
                    n <= completedCount
          ...
          <div className="space-y-4 relative">
            ...
            <div className="absolute right-[31px] top-10 bottom-10 w-1 bg-muted rounded-full overflow-hidden hidden md:block">
              <motion.div
                initial={{ height: 0 }}
                animate={{ height: `${(completedCount / 4) * 100}%` }}
              ...
              return (
                ...
                >
                  <Link
                    ...
                    className={`
                      relative block rounded-2xl p-6 border transition-all duration-300 group
                      ${isActive ? "border-primary bg-primary/5 shadow-lg shadow-primary/10" : ""}
                      ${isCompleted ? "border-border bg-card/50 opacity-70" : ""}
                      ${isLocked ? "border-border/30 bg-card/30 opacity-40 pointer-events-none grayscale" : ""}
                  ...
                  >
                    ...
                    {isLocked && (
                      <div className="absolute top-4 left-4 text-muted-foreground/30">
                    ...
                    <div className="flex items-center gap-5">
                      <div
                        className={`w-14 h-14 rounded-2xl flex items-center justify-center text-white shrink-0 shadow-lg bg-gradient-to-br ${item.color} ${item.shadow} group-hover:scale-105 transition-transform`}
                      >
                        {item.icon}
                      </div>
                      <div className="flex-1 min-w-0">
                        <h3 className="text-lg font-black text-foreground">{item.title}</h3>
                        <p className="text-xs text-primary font-bold">{item.subtitle}</p>
                        <p className="text-xs text-muted-foreground mt-1 line-clamp-1">{item.description}</p>
                      </div>
                      {isActive && (
                        <div className="shrink-0 text-primary font-black text-sm flex items-center gap-1 group-hover:-translate-x-1 transition-transform">
