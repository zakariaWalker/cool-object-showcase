      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-5">
          ...
          <div className="space-y-4">
            <ParamSlider label="a" value={a} onChange={setA} min={-5} max={5} step={0.1} color="text-red-500" desc="يتحكم في فتحة القطع واتجاهه" />
            <ParamSlider label="b" value={b} onChange={setB} min={-10} max={10} step={0.5} color="text-blue-500" desc="يزيح القطع أفقياً" />
            <ParamSlider label="c" value={c} onChange={setC} min={-10} max={10} step={0.5} color="text-green-500" desc="يرفع أو يخفض القطع عمودياً" />
          </div>
          ...
          <div className="p-4 rounded-2xl bg-muted/30 border border-border text-center">
            <div className="text-lg font-mono font-black text-foreground" dir="ltr">
              f(x) = <span className="text-red-500">{a}</span>x² {b >= 0 ? "+" : ""} <span className="text-blue-500">{b}</span>x {c >= 0 ? "+" : ""} <span className="text-green-500">{c}</span>
            </div>
          </div>

          {/* Info cards */}
          <div className="space-y-2">
            <InfoCard label="المميّز Δ" value={discriminant.toFixed(2)} color={discriminant > 0 ? "text-green-500" : discriminant === 0 ? "text-amber-500" : "text-red-500"} />
        ...
        <div className="lg:col-span-2 rounded-2xl border border-border bg-card overflow-hidden">
          <svg viewBox={`0 0 ${W} ${H}`} className="w-full" style={{ maxHeight: 450 }}>
            ...
            {roots.map((r, i) => {
              ...
              return (
                <g key={i}>
                  <circle cx={sx} cy={sy} r={6} className="fill-red-500" />
                  <text x={sx} y={sy + 18} className="fill-red-500 text-[10px] font-bold" textAnchor="middle">
                    {r.x.toFixed(2)}
                  </text>
                </g>
              );
            ...
            {vertex && (() => {
              ...
              return (
                <g>
                  <circle cx={sx} cy={sy} r={5} className="fill-amber-500" />
                  <text x={sx} y={sy - 10} className="fill-amber-500 text-[9px] font-bold" textAnchor="middle">
                    رأس
                  </text>
                </g>
              );
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="space-y-5">
          ...
          <ParamSlider label="الضلع a" value={sideA} onChange={setSideA} min={0.5} max={15} step={0.5} color="text-blue-500" desc="الضلع العمودي" />
          <ParamSlider label="الضلع b" value={sideB} onChange={setSideB} min={0.5} max={15} step={0.5} color="text-green-500" desc="الضلع الأفقي" />

          {/* Pythagorean theorem */}
          <div className="p-4 rounded-2xl bg-muted/30 border border-border text-center font-mono" dir="ltr">
            <div className="text-sm text-foreground">
              <span className="text-blue-500">{sideA}²</span> + <span className="text-green-500">{sideB}²</span> = <span className="text-red-500">{hypotenuse.toFixed(2)}²</span>
            </div>
          ...
          <div className="space-y-2">
            <InfoCard label="الوتر c" value={hypotenuse.toFixed(3)} color="text-red-500" />
        ...
        <div className="lg:col-span-2 rounded-2xl border border-border bg-card overflow-hidden">
          <svg viewBox={`0 0 ${W} ${H}`} className="w-full" style={{ maxHeight: 450 }}>
            ...
            {/* Labels */}
            <text x={(ax + ax) / 2 - 25} y={(ay + cy) / 2} className="fill-blue-500 text-[13px] font-black" textAnchor="middle">a = {sideA}</text>
            <text x={(ax + bx) / 2} y={ay + 25} className="fill-green-500 text-[13px] font-black" textAnchor="middle">b = {sideB}</text>
            <text x={(bx + cx) / 2 + 20} y={(by + cy) / 2} className="fill-red-500 text-[13px] font-black" textAnchor="middle">c = {hypotenuse.toFixed(2)}</text>
            {/* Area squares visualization */}
            <rect x={ax - sideA * scale} y={cy} width={sideA * scale} height={sideA * scale} className="fill-blue-500/10 stroke-blue-500/30" strokeWidth={1} strokeDasharray="3 3" />
            <text x={ax - sideA * scale / 2} y={cy + sideA * scale / 2} className="fill-blue-500/50 text-[10px] font-bold" textAnchor="middle">a²={(sideA*sideA).toFixed(1)}</text>
            <rect x={ax} y={ay} width={sideB * scale} height={sideB * scale * 0.5} className="fill-green-500/10 stroke-green-500/30" strokeWidth={1} strokeDasharray="3 3" opacity={0.5} />
          </svg>
        </div>
      </div>
    </motion.div>
...
function ProbabilityWhatIf() {
  ...
  const balls: { color: string; className: string }[] = [
    ...Array(red).fill({ color: "أحمر", className: "fill-red-500" }),
    ...Array(blue).fill({ color: "أزرق", className: "fill-blue-500" }),
    ...Array(green).fill({ color: "أخضر", className: "fill-green-500" }),
  ];
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="space-y-5">
          ...
          <ParamSlider label="🔴 كرات حمراء" value={red} onChange={v => setRed(Math.round(v))} min={0} max={20} step={1} color="text-red-500" />
          <ParamSlider label="🔵 كرات زرقاء" value={blue} onChange={v => setBlue(Math.round(v))} min={0} max={20} step={1} color="text-blue-500" />
          <ParamSlider label="🟢 كرات خضراء" value={green} onChange={v => setGreen(Math.round(v))} min={0} max={20} step={1} color="text-green-500" />
          ...
          <div className="space-y-2 mt-4">
            ...
            <InfoCard label="P(أحمر)" value={`${(pRed * 100).toFixed(1)}%`} color="text-red-500" />
            <InfoCard label="P(أزرق)" value={`${(pBlue * 100).toFixed(1)}%`} color="text-blue-500" />
            <InfoCard label="P(أخضر)" value={`${(pGreen * 100).toFixed(1)}%`} color="text-green-500" />
            {draws > 1 && (
              <>
                <InfoCard label={`P(≥1 أحمر في ${draws} سحبات)`} value={`${(pAtLeastOneRed * 100).toFixed(1)}%`} color="text-red-500" />
                <InfoCard label={`P(كلها زرقاء في ${draws} سحبات)`} value={`${(pAllBlue * 100).toFixed(1)}%`} color="text-blue-500" />
              </>
            )}
          </div>
        </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="rounded-2xl border border-border bg-card p-4 space-y-3">
            ...
            <ProbBar label="أحمر" percent={pRed * 100} color="bg-red-500" />
            <ProbBar label="أزرق" percent={pBlue * 100} color="bg-blue-500" />
            <ProbBar label="أخضر" percent={pGreen * 100} color="bg-green-500" />
          </div>
        </div>
      </div>
    </motion.div>
