...
import { Upload, Brain, GitBranch, AlertTriangle, FileText, Loader2, X, ChevronDown, ChevronRight, Search, Zap, Link2, BookOpen } from "lucide-react";
...
interface Skill {
  ...
  domain: string; subdomain: string; grade: string;
  difficulty: number; bloom_level: number; frequency: number;
  metadata: any; created_at: string;
}
interface SkillDep { id: string; from_skill_id: string; to_skill_id: string; dependency_type: string; strength: number; }
interface SkillError { id: string; skill_id: string; error_description: string; error_type: string; frequency: number; fix_hint: string; severity: string; }
interface Course { id: string; title: string; description: string; grade: string; source_type: string; file_path: string | null; status: string; extracted_skills: any; user_id: string; created_at: string; }

type View = "upload" | "graph" | "skills" | "errors" | "courses";

const DOMAIN_COLORS: Record<string, string> = {
  algebra: "#6366f1", geometry: "#f59e0b", statistics: "#10b981",
  probability: "#ef4444", functions: "#8b5cf6", other: "#64748b",
};

const DOMAIN_ICONS: Record<string, string> = {
  algebra: "∑", geometry: "△", statistics: "📊",
  probability: "🎲", functions: "ƒ",
};
  ...
  return (
    <div className="min-h-screen bg-background" dir="rtl">
      ...
      <div className="max-w-7xl mx-auto p-4">
        {view === "upload" && <UploadPanel onDone={() => { loadData(); setView("skills"); }} />}
        {view === "skills" && <SkillsList skills={skills} errors={errors} deps={deps} selected={selectedSkill} onSelect={setSelectedSkill} />}
...
function QuickStats({ skills, errors, deps }: { skills: Skill[]; errors: SkillError[]; deps: SkillDep[] }) {
  const domains = useMemo(() => {
    ...
    skills.forEach(s => { m[s.domain] = (m[s.domain] || 0) + 1; });
    return m;
  }, [skills]);
...
function UploadPanel({ onDone }: { onDone: () => void }) {
  ...
  const handleExtract = async () => {
    ...
    if (mode === "pdf" && pdfFile) {
      ...
      if (!user) { toast.error("سجل الدخول أولاً"); setProcessing(false); return; }

      const path = `courses/${user.id}/${Date.now()}_${pdfFile.name}`;
      const { error: upErr } = await supabase.storage.from("educational-materials").upload(path, pdfFile);
      if (upErr) { toast.error("خطأ في الرفع: " + upErr.message); setProcessing(false); return; }

      setProgress(30);
      toast.info("جاري تحليل PDF...");

      // Use parse-exam-pdf to extract text
      const { data: parseData, error: parseErr } = await supabase.functions.invoke("parse-exam-pdf", {
        body: { file_path: path, bucket: "educational-materials" },
      });

      if (parseErr || !parseData) {
        toast.error("خطأ في تحليل PDF");
        setProcessing(false);
        return;
      }

      content = parseData.text || parseData.extracted_text || JSON.stringify(parseData);
      setProgress(50);
    ...
    const { data: course, error: courseErr } = await supabase.from("kb_courses").insert({
      title, grade, description: content.slice(0, 500),
      source_type: mode, user_id: user.id, status: "processing",
      file_path: mode === "pdf" ? `courses/${user.id}/${pdfFile?.name}` : null,
    }).select("id").single();

    if (courseErr) { toast.error("خطأ في إنشاء الكورس"); setProcessing(false); return; }
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-3xl mx-auto space-y-6">
      <div className="bg-card border border-border rounded-xl p-6">
        ...
        <p className="text-sm text-muted-foreground mb-4">ارفع كورس أو منهج ليتم تفكيكه إلى مهارات دقيقة مع الأخطاء الشائعة</p>
        ...
        {mode === "pdf" ? (
          ...
          <Textarea placeholder="الصق محتوى الدرس أو المنهج هنا..." rows={10}
        ...
        {processing && (
          <div className="mt-4 space-y-2">
            ...
            <p className="text-xs text-muted-foreground text-center">
              {progress < 30 ? "رفع الملف..." : progress < 60 ? "تحليل المحتوى..." : "استخراج المهارات بالذكاء الاصطناعي..."}
            </p>
          </div>
        )}
...
// ─── Skills List ───
function SkillsList({ skills, errors, deps, selected, onSelect }: {
  skills: Skill[]; errors: SkillError[]; deps: SkillDep[];
  selected: Skill | null; onSelect: (s: Skill | null) => void;
}) {
      ...
      <div className="flex-1 space-y-3">
        <div className="flex gap-2">
          <div className="relative flex-1">
            ...
            <Input placeholder="ابحث عن مهارة..." value={search} onChange={e => setSearch(e.target.value)} className="pr-9 text-sm" />
          </div>
          <Select value={domainFilter} onValueChange={setDomainFilter}>
            <SelectTrigger className="w-36"><SelectValue /></SelectTrigger>
            <SelectContent>
              <SelectItem value="all">الكل</SelectItem>
              {Object.keys(DOMAIN_COLORS).filter(d => d !== "other").map(d => (
                <SelectItem key={d} value={d}>{DOMAIN_ICONS[d]} {d}</SelectItem>
              ))}
            </SelectContent>
          </Select>
        </div>
              ...
              <motion.div key={skill.id} layout
                ...
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex items-center gap-1.5">
                    <Badge variant="secondary" className="text-[10px]">B{skill.bloom_level}</Badge>
                    <Badge variant="outline" className="text-[10px]">D{skill.difficulty}</Badge>
                    {sErrors.length > 0 && (
        ...
        {selected && (
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}
            ...
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => onSelect(null)}><X size={14} /></button>
            </div>
            ...
            <div className="grid grid-cols-3 gap-2 text-center">
              <div className="bg-background rounded-lg p-2 border border-border">
                <div className="text-lg font-bold" style={{ color: DOMAIN_COLORS[selected.domain] }}>{DOMAIN_ICONS[selected.domain]}</div>
                ...
                {errorsForSkill(selected.id).map(err => (
                  <div key={err.id} className="bg-destructive/5 border border-destructive/10 rounded-lg p-2 mb-1.5">
                    ...
                    <div className="text-[10px] text-muted-foreground mt-0.5">
                      <Badge variant="outline" className="text-[9px] mr-1">{err.error_type}</Badge>
                      <Badge variant={err.severity === "high" ? "destructive" : "secondary"} className="text-[9px]">{err.severity}</Badge>
                    </div>
            ...
            {depsForSkill(selected.id).length > 0 && (
              <div>
                ...
                {depsForSkill(selected.id).map(dep => {
                  ...
                  return target ? (
                    <div key={dep.id} className="text-xs bg-background border border-border rounded-lg p-2 mb-1 cursor-pointer hover:border-primary/30"
  ...
  function errorsForSkill(id: string) { return errors.filter(e => e.skill_id === id); }
  function depsForSkill(id: string) { return deps.filter(d => d.from_skill_id === id); }
}

// ─── Skill Graph (2D SVG) ───
function SkillGraph({ skills, deps, errors, onSelect }: {
  skills: Skill[]; deps: SkillDep[]; errors: SkillError[]; onSelect: (s: Skill) => void;
}) {
  ...
  const { nodes, edges } = useMemo(() => {
    ...
    // Layout: group by domain, then arrange in columns
    ...
    const edgeList = deps.filter(d => nodePositions[d.from_skill_id] && nodePositions[d.to_skill_id]).map(d => ({
  ...
  return (
    <div className="bg-card border border-border rounded-xl overflow-auto">
      <svg width={svgWidth} height={svgHeight} className="w-full" style={{ minHeight: 400 }}>
        ...
        {/* Domain headers */}
        {Object.keys(DOMAIN_COLORS).filter(d => d !== "other" && skills.some(s => s.domain === d)).map((domain, di) => (
          <g key={domain}>
            <text x={40 + di * 220 + 110} y={30} textAnchor="middle" fontSize="13" fontWeight="bold"
              fill={DOMAIN_COLORS[domain]}>{DOMAIN_ICONS[domain]} {domain}</text>
          </g>
        ))}
        ...
        {edges.map((e, i) => (
          <line key={i} x1={e.from.x} y1={e.from.y} x2={e.to.x} y2={e.to.y}
            stroke="hsl(var(--muted-foreground))" strokeWidth={1} strokeOpacity={0.25}
            markerEnd="url(#arrow)" strokeDasharray={e.type === "enhances" ? "4,4" : undefined} />
        ))}
        ...
        {nodes.map(node => {
          ...
          return (
            <g key={node.id} transform={`translate(${node.x}, ${node.y})`}
              ...
              onClick={() => onSelect(node.skill)} className="cursor-pointer">
              <rect x={-90} y={-18} width={180} height={36} rx={8}
                fill={isHovered ? color : "hsl(var(--card))"}
                stroke={color} strokeWidth={isHovered ? 2 : 1}
                opacity={isHovered ? 1 : 0.85} />
              <text x={0} y={1} textAnchor="middle" fontSize="10" fontWeight="600"
                fill={isHovered ? "white" : "hsl(var(--foreground))"} dominantBaseline="middle">
...
function ErrorsPanel({ errors, skills }: { errors: SkillError[]; skills: Skill[] }) {
  ...
  const byType = useMemo(() => {
    const m: Record<string, number> = {};
    errors.forEach(e => { m[e.error_type] = (m[e.error_type] || 0) + 1; });
    return m;
  }, [errors]);
      ...
      {grouped.map(({ skill, errors: errs }) => (
        <div key={skill.id} className="bg-card border border-border rounded-xl p-4">
          <h3 className="font-bold text-sm text-foreground mb-2 flex items-center gap-2">
            ...
            <Badge variant="destructive" className="text-[10px]">{errs.length} أخطاء</Badge>
          </h3>
          <div className="grid gap-2">
            {errs.map(err => (
              <div key={err.id} className="bg-destructive/5 border border-destructive/10 rounded-lg p-3 flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <Badge variant={err.severity === "high" ? "destructive" : err.severity === "medium" ? "secondary" : "outline"} className="text-[9px]">{err.severity}</Badge>
                  </div>
          ...
          <div key={course.id} className="bg-card border border-border rounded-xl p-4">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <Badge variant={course.status === "completed" ? "default" : course.status === "processing" ? "secondary" : "outline"}>
