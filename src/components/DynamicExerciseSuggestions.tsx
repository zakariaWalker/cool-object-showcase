    ...
    try {
      ...
      for (const offset of offsets) {
        const { data } = await supabase
          .from("kb_exercises")
          .select("id, text, grade, chapter, type, source")
          .range(offset, offset)
          .limit(1);
        if (data && data[0]) {
          results.push({
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
        ...
        <button
          ...
          style={{
            ...
            color: "hsl(var(--primary))", padding: "4px 12px",
        ...
        {suggestions.map(ex => {
          ...
          return (
            <button
              ...
              onMouseEnter={e => { (e.currentTarget as HTMLElement).style.transform = "translateY(-2px)"; (e.currentTarget as HTMLElement).style.boxShadow = `0 4px 12px ${style.color}20`; }}
              onMouseLeave={e => { (e.currentTarget as HTMLElement).style.transform = ""; (e.currentTarget as HTMLElement).style.boxShadow = ""; }}
            >
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                <span style={{ fontSize: 18 }}>{style.emoji}</span>
                <span style={{ fontSize: 10, fontWeight: 700, color: style.color, background: `${style.color}15`, padding: "2px 8px", borderRadius: 10 }}>
                  {gradeLabel(ex.grade)}
                </span>
              </div>
              <div style={{
